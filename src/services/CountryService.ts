import { CountryRowValue, Value } from '@/types'
...
const CountryService = {
  ...
  async getCountries({ indicator }: { indicator: string }): Promise<CountryRowValue[]> {
    ...
    ARRAY(SELECT JSON_BUILD_OBJECT('id', v.id, 'value', v.value, 'year', v.year)
     ...
     as values FROM "Country" c WHERE c."hidden" IS FALSE AND (SELECT COUNT("id") FROM "Value" WHERE "indicatorId" = ${indicator} AND "countryId" = c."id" AND "year" <= ${new Date().getFullYear()}) > 0 ORDER BY "name" ASC`
  },

  async getCountriesValueByIndicator({ indicator }: { indicator: string }): Promise<CountryRowValue[]> {
