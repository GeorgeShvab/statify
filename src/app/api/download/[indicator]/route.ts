...
export const GET = async (req: NextRequest, { params }: { params: Params }) => {
  ...
  const countryValuesPromise = CountryService.getCountriesValueByIndicator({ indicator: params.indicator })

  const [indicator, countryValues] = await Promise.all([indicatorPromise, countryValuesPromise])

  if (!indicator || !countryValues.length) return new NextResponse(null, { status: 404 })
  ...
  if (format === 'xlsx') {
    ...
    const header = 'Region,Year,Value\n'
    const data = countryValues.map((item) => [item.name, item.year, item.value].join(',')).join('\n')
